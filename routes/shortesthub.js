...
router.post('/findHub', async (req, res) => {
  ...
  let spokeIds = await Shipment.find({ date: { $eq: date } },{source:1,destination:1,_id:0});
  ...
  let hub = await selectBestHub(spokeIds);
  //Updates the Shipment Hub and status in MongoDB
  Shipment.updateMany({ date: { $eq: date } }, { $set: { hub:hub,status:statuses.ONGOINGTOHUB } }, (err, result) => {
    if (err) {
      console.error(err);
    } else {
